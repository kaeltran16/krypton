from contextlib import asynccontextmanager
from unittest.mock import AsyncMock, MagicMock

import pytest
from fastapi import FastAPI
from httpx import ASGITransport, AsyncClient

from app.api.routes import create_router
from tests.conftest import make_test_jwt


def _mock_db(scalars_all=None, scalar_result=None):
    mock_session = AsyncMock()
    mock_result = MagicMock()
    mock_result.scalars.return_value.all.return_value = scalars_all or []
    mock_result.scalar.return_value = scalar_result
    mock_session.execute = AsyncMock(return_value=mock_result)

    mock_db = MagicMock()

    @asynccontextmanager
    async def fake_session():
        yield mock_session

    mock_db.session_factory = fake_session
    return mock_db


@pytest.fixture
def app_with_routes():
    app = FastAPI()
    mock_settings = MagicMock()
    mock_settings.jwt_secret = "test-jwt-secret"
    app.state.settings = mock_settings
    app.state.db = _mock_db()
    app.state.manager = MagicMock()
    app.include_router(create_router())
    return app


@pytest.mark.asyncio
async def test_signals_outcome_filter(app_with_routes):
    """GET /signals?outcome=resolved should return 200 with list."""
    async with AsyncClient(
        transport=ASGITransport(app=app_with_routes), base_url="http://test"
    ) as client:
        resp = await client.get(
            "/api/signals",
            params={"outcome": "resolved"},
            cookies={"krypton_token": make_test_jwt()},
        )
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


@pytest.mark.asyncio
async def test_signals_outcome_specific_value(app_with_routes):
    """GET /signals?outcome=TP1_HIT should return 200."""
    async with AsyncClient(
        transport=ASGITransport(app=app_with_routes), base_url="http://test"
    ) as client:
        resp = await client.get(
            "/api/signals",
            params={"outcome": "TP1_HIT"},
            cookies={"krypton_token": make_test_jwt()},
        )
    assert resp.status_code == 200
    assert isinstance(resp.json(), list)


@pytest.mark.asyncio
async def test_signals_count_endpoint(app_with_routes):
    """GET /signals/count should return 200 with count."""
    app_with_routes.state.db = _mock_db(scalar_result=42)
    async with AsyncClient(
        transport=ASGITransport(app=app_with_routes), base_url="http://test"
    ) as client:
        resp = await client.get(
            "/api/signals/count",
            params={"pair": "BTC-USDT-SWAP", "outcome": "resolved"},
            cookies={"krypton_token": make_test_jwt()},
        )
    assert resp.status_code == 200
    data = resp.json()
    assert data["count"] == 42
