...
PARAM_GROUPS: dict[str, dict] = {
    ...
    "regime_outer": {
        ...
        "sweep_ranges": {
            f"{r}_{src}_weight": (
                (0.0, 0.20, None) if src in ("liquidation", "confluence") else (0.10, 0.50, None)
            )
            for r in REGIMES
            for src in OUTER_KEYS
        },
...
def _confluence_ok(c: dict[str, Any]) -> bool:
    w1 = c.get("level_weight_1", 0.5)
    w2 = c.get("level_weight_2", 0.3)
    w3 = 1.0 - w1 - w2
    return (
        w1 + w2 < 1.0
        and w3 >= 0.05
...
PARAM_GROUPS["confluence"] = {
    ...
    "constraints": _confluence_ok,
    "priority": _priority_for("confluence"),
}
